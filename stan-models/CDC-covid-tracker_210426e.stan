...
model {
  ...
  for(w in 1:W_OBSERVED){
    ...
    for(b in idx_non_missing[1:N_idx_non_missing[w],w]){
      if(deaths[b,w] == 0)
        target += log_sum_exp(bernoulli_lpmf(1 | rho), bernoulli_lpmf(0 | rho) + neg_binomial_lpmf(deaths[b,w] | alpha_reduced[b, IDX_WEEKS_OBSERVED[w]] , theta ));
      else
        target += bernoulli_lpmf(0 | rho) + neg_binomial_lpmf(deaths[b,w] | alpha_reduced[b, IDX_WEEKS_OBSERVED[w]] , theta );
    }
  ...
  for(n in 1:N_missing){
    if(!start_or_end_period[n])
    {
      ...
      // print("n", n);
      // print("sum_count_censored[n]", sum_count_censored[n]);
      // print("age_missing[n]", age_missing[n]);
      // print("idx_weeks_missing[1:N_weeks_missing[n], n]", idx_weeks_missing[1:N_weeks_missing[n], n]);
      // print("theta", theta);
      // print("sum(alpha_reduced[age_missing[n], idx_weeks_missing[1:N_weeks_missing[n], n] ])", sum(alpha_reduced[age_missing[n], idx_weeks_missing[1:N_weeks_missing[n], n] ]));
      // print("alpha_reduced[age_missing[n], idx_weeks_missing[1:N_weeks_missing[n], n] ]", alpha_reduced[age_missing[n], idx_weeks_missing[1:N_weeks_missing[n], n] ]);
      // print("phi_reduced", phi_reduced[ age_missing[n], idx_weeks_missing[1:N_weeks_missing[n], n] ]);
      // print("log lik", neg_binomial_lpmf( sum_count_censored[n] | sum(alpha_reduced[age_missing[n], idx_weeks_missing[1:N_weeks_missing[n], n] ]) , theta ));
      // print("log lik 2", neg_binomial_lpmf( sum_count_censored[n] | 5 , theta ));
