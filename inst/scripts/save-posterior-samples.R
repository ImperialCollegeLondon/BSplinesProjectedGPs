...
if(nrow(do)!=0){
  ...
  if("lambda" %in% vars) names(dimnames(re[["lambda"]])) = c('iterations', rep('', length(dim(re[["lambda"]])) - 1))
  if("phi" %in% vars) names(dimnames(re[["phi"]])) = c('iterations', rep('', length(dim(re[["phi"]])) - 1))
  if("alpha" %in% vars) names(dimnames(re[["alpha"]])) = c('iterations', rep('', length(dim(re[["alpha"]])) - 1))
  if("alpha_reduced" %in% vars) names(dimnames(re[["alpha_reduced"]])) = c('iterations', rep('', length(dim(re[["alpha_reduced"]])) - 1))
  if("alpha_reduced_vac" %in% vars) names(dimnames(re[["alpha_reduced_vac"]])) = c('iterations', rep('', length(dim(re[["alpha_reduced_vac"]])) - 1))
  if("phi_reduced" %in% vars) names(dimnames(re[["phi_reduced"]])) = c('iterations', rep('', length(dim(re[["phi_reduced"]])) - 1))
  if("phi_reduced_vac" %in% vars) names(dimnames(re[["phi_reduced_vac"]])) = c('iterations', rep('', length(dim(re[["phi_reduced_vac"]])) - 1))
  if("phi_predict_reduced_vac" %in% vars) names(dimnames(re[["phi_predict_reduced_vac"]])) = c('iterations', rep('', length(dim(re[["phi_predict_reduced_vac"]])) - 1))
  if("f" %in% vars) names(dimnames(re[["f"]])) = c('iterations', rep('', length(dim(re[["f"]])) - 1))
  if("E_pdeaths" %in% vars) names(dimnames(re[["E_pdeaths"]])) = c('iterations', rep('', length(dim(re[["E_pdeaths"]])) - 1))
  if("r_pdeaths" %in% vars) names(dimnames(re[["r_pdeaths"]])) = c('iterations', rep('', length(dim(re[["r_pdeaths"]])) - 1))
  if("log_lik" %in% vars) names(dimnames(re[["log_lik"]])) = c('iterations', rep('', length(dim(re[["log_lik"]])) - 1))
  if("deaths_predict" %in% vars) names(dimnames(re[["deaths_predict"]])) = c('iterations', rep('', length(dim(re[["deaths_predict"]])) - 1))
  if("deaths_predict_state_age_strata" %in% vars) names(dimnames(re[["deaths_predict_state_age_strata"]])) = c('iterations', rep('', length(dim(re[["deaths_predict_state_age_strata"]])) - 1))
  if("deaths_predict_vac_age_strata" %in% vars) names(dimnames(re[["deaths_predict_vac_age_strata"]])) = c('iterations', rep('', length(dim(re[["deaths_predict_vac_age_strata"]])) - 1))
  if("log_r_pdeaths_predict" %in% vars) names(dimnames(re[["log_r_pdeaths_predict"]])) = c('iterations', rep('', length(dim(re[["log_r_pdeaths_predict"]])) - 1))
  if("r_pdeaths_predict" %in% vars) names(dimnames(re[["r_pdeaths_predict"]])) = c('iterations', rep('', length(dim(re[["r_pdeaths_predict"]])) - 1))
  if("E_pdeaths_predict" %in% vars) names(dimnames(re[["E_pdeaths_predict"]])) = c('iterations', rep('', length(dim(re[["E_pdeaths_predict"]])) - 1))
  if("E_pdeaths_predict_resurgence_cumulative" %in% vars) names(dimnames(re[["E_pdeaths_predict_resurgence_cumulative"]])) = c('iterations', rep('', length(dim(re[["E_pdeaths_predict_resurgence_cumulative"]])) - 1))
  if("E_pdeaths_counterfactual" %in% vars) names(dimnames(re[["E_pdeaths_counterfactual"]])) = c('iterations', rep('', length(dim(re[["E_pdeaths_counterfactual"]])) - 1))
  if("E_pdeaths_counterfactual_resurgence_cumulative" %in% vars) names(dimnames(re[["E_pdeaths_counterfactual_resurgence_cumulative"]])) = c('iterations', rep('', length(dim(re[["E_pdeaths_counterfactual_resurgence_cumulative"]])) - 1))
  if("diff_E_pdeaths_counterfactual" %in% vars) names(dimnames(re[["diff_E_pdeaths_counterfactual"]])) = c('iterations', rep('', length(dim(re[["diff_E_pdeaths_counterfactual"]])) - 1))
  if("perc_E_pdeaths_counterfactual" %in% vars) names(dimnames(re[["perc_E_pdeaths_counterfactual"]])) = c('iterations', rep('', length(dim(re[["perc_E_pdeaths_counterfactual"]])) - 1))
