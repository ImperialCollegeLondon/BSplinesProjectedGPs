...
model {
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      r_pdeaths[m][c,:] ~ exponential(rep_vector(1, T) ./  xi[c][:,m] );
    }
  }
...
generated quantities {
  ...
  matrix[C,T] perc_E_pdeaths_counterfactual[M]; 
  {
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            r_pdeaths_predict[m][c,:] = to_row_vector( exponential_rng( rep_vector(1, T) ./  xi[c][:,m] ) );
            log_r_pdeaths_predict[m][c,:] = log( r_pdeaths_predict[m][c,:] );
            
            r_pdeaths_counterfactual[m][c,:] = to_row_vector( exponential_rng( rep_vector(1, T) ./ xi_counterfactual[c][:,m] ) );
