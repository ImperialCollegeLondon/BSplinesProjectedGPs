...
data{
  ...
  matrix[A,W] prop_vaccinated_other;
...
transformed parameters {
  ...
  matrix[A, W] other = rep_matrix(append_row(rep_vector(0, max_age_not_vaccinated), delta[map_A_to_C]), W) .* prop_vaccinated_other;
  

  for(w in 1:W)
  {
    
    phi[:,w] = softmax( f[:,w] ) .* (rep_vector(1, A) + susceptible[:,w]) .* (rep_vector(1, A) + other[:,w]); 
