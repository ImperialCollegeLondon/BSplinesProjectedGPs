functions {
  ...
  matrix get_reduced_matrix(int B, int W, matrix matrix_to_reduce, matrix state_age_strata){
   ...
   matrix get_expected_resurgence_deaths(int C, int T, vector prop_vac_start, vector week_indices_resurgence,
    ...
    vector[C] intercept_resurgence = intercept_resurgence0 + intercept_resurgence_re + prop_vac_start .* vaccine_effect_intercept_diagonal;
    vector[C] slope_resurgence = slope_resurgence0 + prop_vac_start .* vaccine_effect_slope_diagonal;
    
    matrix[C,T] log_xi = rep_matrix(intercept_resurgence, T) + (rep_matrix(to_row_vector(week_indices_resurgence),C) .* rep_matrix(slope_resurgence, T));
    matrix[C,T] xi = exp(log_xi);
...
generated quantities{
  ...
  matrix[C,T] diff_E_pdeaths_counterfactual[N_COUNTERFACTUAL] = rep_array(rep_matrix(1.0, C, T), N_COUNTERFACTUAL);
  matrix[C,T] perc_E_pdeaths_counterfactual[N_COUNTERFACTUAL] = rep_array(rep_matrix(1.0, C, T), N_COUNTERFACTUAL);
    ...
    int m = LOCATION_PROCESSING_IDX;
      ...
      log_xi = log(xi);
        ...
        for(w in 1:W_OBSERVED){
            for(i in idx_non_missing[m][1:N_idx_non_missing[m][w],w]){
                idx_log_lik += 1;
                log_lik[idx_log_lik] = neg_binomial_lpmf(deaths[m,i,w] | alpha_reduced[i, IDX_WEEKS_OBSERVED[w]] , nu_inverse );
            }
        }
        ...
        for(n in 1:N_missing[m]){
            ...
            if(!start_or_end_period[m][n])
                {
                idx_log_lik += 1;
                log_lik[idx_log_lik] = neg_binomial_lpmf( sum_count_censored[m][n] | alpha_reduced_missing , nu_inverse ) ;
            } else {
                for(i in min_count_censored[m][n]:max_count_censored[m][n]){
                    idx_log_lik += 1;
                    log_lik[idx_log_lik] += my_neg_binomial_lpmf( i | alpha_reduced_missing , nu_inverse ) ;
                }
            }
        }
    }
  ...
  
  {
    ...
    for(c in 1:C){
      ...
      log_r_pdeaths_predict[c,:] = to_row_vector( skew_normal_rng(log_xi[c,1:T],sigma_r_pdeaths[m],skewness_r_pdeaths[m] ));
  ...
  for(n in 1:N_COUNTERFACTUAL){
    int m = LOCATION_PROCESSING_IDX;
    matrix[C,T] log_r_pdeaths_counterfactual = rep_matrix(0.0, C, T);
    matrix[C,T] r_pdeaths_counterfactual = rep_matrix(1.0, C, T);
    matrix[C,T] xi_counterfactual;
    matrix[C,T] log_xi_counterfactual;
  
    xi_counterfactual = get_expected_resurgence_deaths(C, T, to_vector(prop_vac_start_counterfactual[:,n,m]), week_indices_resurgence,
             to_vector(intercept_resurgence0), to_vector(intercept_resurgence_re[:,m]), to_vector(vaccine_effect_intercept_diagonal),
            to_vector(slope_resurgence0), to_vector(vaccine_effect_slope_diagonal));
    log_xi_counterfactual = log(xi_counterfactual);

      for(c in 1:C){
          log_r_pdeaths_counterfactual[c,1:T] = to_row_vector( skew_normal_rng( log_xi_counterfactual[c,1:T], sigma_r_pdeaths[m], skewness_r_pdeaths[m]));
          ...
          perc_E_pdeaths_counterfactual[n][c,:] = diff_E_pdeaths_counterfactual[n][c,:] ./ E_pdeaths_predict_resurgence_cumulative[c,:] ;
