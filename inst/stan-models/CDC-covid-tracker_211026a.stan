...
parameters {
  ...
  row_vector<lower=0>[M] nu_pdeaths_unscaled;
}
...
transformed parameters {
  ...
  matrix[T, M] xi[C];
  matrix[T, M] alpha_pdeaths[C];
  row_vector<lower=0>[M] nu_pdeaths;
  row_vector<lower=0>[M] theta_pdeaths;
    ...
    for(w in 1:W){
      ...
      for(c in 1:C){
        ...
        if(w >= w_start_resurgence && w <= w_stop_resurgence){
            ...
            log_r_pdeaths[m][c,w - w_start_resurgence + 1] = ( r_pdeaths[m][c,w - w_start_resurgence + 1] );
        }
  ...
  // vaccination model
  nu_pdeaths = rep_row_vector(1, M).*nu_pdeaths_unscaled;
  theta_pdeaths = rep_row_vector(1, M) ./ nu_pdeaths;
  ...
  for(c in 1:C){
    ...
    alpha_pdeaths[c] = xi[c] ./ rep_matrix(nu_pdeaths, T);
  }
...
model {
  ...
  nu_pdeaths_unscaled ~ cauchy(0,1);

  for(c in 1:C){
    intercept_resurgence_re[c] ~ normal(intercept_resurgence0[c],sigma_intercept_resurgence[c]);
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      log_r_pdeaths[m][c,:] ~ normal(xi[c][:,m], nu_pdeaths[m]);
    }
  }
...
generated quantities {
  ...
  matrix[T, M] alpha_pdeaths_counterfactual[C];
  ...
  matrix[C,T] perc_E_pdeaths_counterfactual[M]; 
  {
    ...
    for(c in 1:C){
        ...
        alpha_pdeaths_counterfactual[c] = xi_counterfactual[c] ./ rep_matrix(nu_pdeaths, T);
    }
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            log_r_pdeaths_predict[m][c,:] = to_row_vector( normal_rng(xi[c][:,m], nu_pdeaths[m]) );

            log_r_pdeaths_counterfactual[m][c,:] = to_row_vector( normal_rng(xi_counterfactual[c][:,m], nu_pdeaths[m]));
            E_pdeaths_counterfactual[m][c,:] = rep_row_vector(E_pdeaths_before_resurgence[m,c], T) + ( log_r_pdeaths_counterfactual[m][c,:] ) ;
