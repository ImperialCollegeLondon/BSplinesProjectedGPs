...
data{
  ...
  int<lower=1,upper=T> T_m[M]; // number of weeks during the Summer 2021 resurgences (w_stop_resurgence-w_start_resurgence+1)
...
model {
  ...
  intercept_resurgence0 ~ normal(-0.69,10);
  slope_resurgence0 ~ normal(0,10);
  
  vaccine_effect_intercept_diagonal ~ normal(0,10);
  vaccine_effect_slope_diagonal ~ normal(0,10);
  vaccine_effect_intercept_cross ~ normal(0,10);
  vaccine_effect_slope_cross ~ normal(0,10);
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      r_pdeaths[m][c,1:T_m[m]] ~ gamma(square(xi[c][1:T_m[m],m] / sigma_r_pdeaths[m]), xi[c][1:T_m[m],m] ./ rep_vector(square(sigma_r_pdeaths[m]), T_m[m]) );
    }
  }
...
generated quantities {
  ...
  matrix[C,T] E_pdeaths_counterfactual[M] = rep_array(rep_matrix(1.0, C, T), M);
  matrix[C,T] E_pdeaths_counterfactual_resurgence_cumulative[M] = rep_array(rep_matrix(1.0, C, T), M);
  ...
  // matrix[C,T] E_pdeaths_resurgence_cumulative[M] = rep_array(rep_matrix(1.0, C, T), M);
  matrix[C,T] diff_E_pdeaths_counterfactual[M] = rep_array(rep_matrix(1.0, C, T), M);
  matrix[C,T] perc_E_pdeaths_counterfactual[M] = rep_array(rep_matrix(1.0, C, T), M);
  matrix[C,min(T_m)] E_pdeaths_resurgence_cumulative_all = rep_matrix(0, C, min(T_m));
  matrix[C,min(T_m)] diff_E_pdeaths_counterfactual_all = rep_matrix(0, C, min(T_m));
  matrix[C,min(T_m)] perc_E_pdeaths_counterfactual_all;
  {
    int idx_log_lik = 0;
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            ...
            diff_E_pdeaths_counterfactual_all[c,:] += diff_E_pdeaths_counterfactual[m][c,1:min(T_m)];
            E_pdeaths_resurgence_cumulative_all[c,:] += E_pdeaths_predict_resurgence_cumulative[m][c,1:min(T_m)];
