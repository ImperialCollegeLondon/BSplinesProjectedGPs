    ...
    for(w in 1:W){
      ...
      for(c in 1:C){
        ...
        if(w >= w_start_resurgence[m] && w <= w_stop_resurgence[m]){
            r_pdeaths[m][c,w - w_start_resurgence[m] + 1] = sum( E_pdeaths[m][c,w_start_resurgence[m]:w] ) / max(E_pdeaths[m][c,1:(w_start_resurgence[m] - 1)]) ;
...
model {
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      log_r_pdeaths[m][c,:] ~ normal(xi[c][:,m], sigma_r_pdeaths[m]);
    }
  }
  ...
  matrix[C,T] perc_E_pdeaths_counterfactual[M]; 
  {
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            ...
            E_pdeaths_counterfactual[m][c,1] = max(E_pdeaths[m][c,1:(w_start_resurgence[m] - 1)]) .* exp( log_r_pdeaths_counterfactual[m][c,1] ) ;
            for(t in 2:T){
               E_pdeaths_counterfactual[m][c,t] = max(E_pdeaths[m][c,1:(w_start_resurgence[m] - 1)]) .* exp( log_r_pdeaths_counterfactual[m][c,t] ) - sum(E_pdeaths_counterfactual[m][c,1:(t-1)]);
            }
