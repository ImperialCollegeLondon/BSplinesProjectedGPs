...
generated quantities{
  ...
  
  {
    ...
    for(c in 1:C){
      ...
      r_pdeaths_predict[c,:] = to_row_vector( gamma_rng(square(xi[c,1:T] / sigma_r_pdeaths[m]), xi[c,1:T] ./ rep_row_vector(square(sigma_r_pdeaths[m]), T)  ) );
  ...
  for(n in 1:N_COUNTERFACTUAL){
    ...
    xi_counterfactual = get_expected_resurgence_deaths(C, T, to_vector(prop_vac_start_counterfactual[:,n,m]), week_indices_resurgence,
      ...
      for(c in 1:C){
          r_pdeaths_counterfactual[c,1:T] = to_row_vector( gamma_rng( square(xi_counterfactual[c,1:T] / sigma_r_pdeaths[m]), xi_counterfactual[c,1:T] ./ rep_row_vector(square(sigma_r_pdeaths[m]), T)) );
