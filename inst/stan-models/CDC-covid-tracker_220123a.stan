...
generated quantities {
  ...
  matrix[C,min(T_m)] E_pdeaths_predict_resurgence_cumulative_all = rep_matrix(1.0, C, min(T_m));
  ...
  matrix[C,min(T_m)] diff_E_pdeaths_counterfactual_all[N_COUNTERFACTUAL] = rep_array(rep_matrix(1.0, C, min(T_m)), N_COUNTERFACTUAL);
  matrix[C,min(T_m)] perc_E_pdeaths_counterfactual_all[N_COUNTERFACTUAL];
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      ...
      E_pdeaths_predict_resurgence_cumulative_all[c,:] += E_pdeaths_predict_resurgence_cumulative[m][c,1:min(T_m)];
  ...
  for(n in 1:N_COUNTERFACTUAL){
    ...
    for(m in 1:M){
      ...
      for(c in 1:C){
          ...
          diff_E_pdeaths_counterfactual_all[n][c,:] += diff_E_pdeaths_counterfactual[n,m][c,1:min(T_m)];
