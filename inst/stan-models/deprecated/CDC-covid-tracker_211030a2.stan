...
model {
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      r_pdeaths[m][c,:] ~ lognormal(log_xi[c][:,m] , sigma_r_pdeaths[c]) ;
    }
  }
...
generated quantities {
  ...
  matrix[C,T] perc_E_pdeaths_counterfactual[M]; 
  {
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            r_pdeaths_predict[m][c,:] = to_row_vector( lognormal_rng(log_xi[c][:,m], sigma_r_pdeaths[c] ) );
            log_r_pdeaths_predict[m][c,:] = log( r_pdeaths_predict[m][c,:] );
            
            r_pdeaths_counterfactual[m][c,:] = to_row_vector( lognormal_rng( log_xi_counterfactual[c][:,m] , sigma_r_pdeaths[c] ) );
