...
transformed parameters {
  ...
  // print("log_xi[c]", log_xi[1]);
  // print("xi[c]", xi[1]);
  // print("sigma_r_pdeaths[1]", sigma_r_pdeaths[1]);
...
model {
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      r_pdeaths[m][c,:] ~ gamma(square(xi[c][:,m] / sigma_r_pdeaths[c]), xi[c][:,m] ./ rep_vector(square(sigma_r_pdeaths[c]), T) );
    }
  }
...
generated quantities {
  ...
  matrix[C,T] perc_E_pdeaths_counterfactual[M];
  {
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            r_pdeaths_predict[m][c,:] = to_row_vector( gamma_rng(square(xi[c][:,m] / sigma_r_pdeaths[c]), xi[c][:,m] ./ rep_vector(square(sigma_r_pdeaths[c]), T)  ) );
            log_r_pdeaths_predict[m][c,:] = log( r_pdeaths_predict[m][c,:] );

            r_pdeaths_counterfactual[m][c,:] = to_row_vector( gamma_rng( square(xi_counterfactual[c][:,m] / sigma_r_pdeaths[c]), xi_counterfactual[c][:,m] ./ rep_vector(square(sigma_r_pdeaths[c]), T)) );
            log_r_pdeaths_counterfactual[m][c,:] = log( r_pdeaths_counterfactual[m][c,:] );

            E_pdeaths_counterfactual[m][c,:] = rep_row_vector( E_pdeaths_before_resurgence[m,c], T) .* r_pdeaths_counterfactual[m][c,:] ;
