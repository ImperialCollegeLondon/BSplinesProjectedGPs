    ...
    for(w in 1:W){
      ...
      for(c in 1:C){
        ...
        if(w >= w_start_resurgence[m] && w <= w_stop_resurgence[m]){
            r_pdeaths[m][c,w - w_start_resurgence[m] + 1] = ( E_pdeaths[m][c,w] - E_pdeaths[m][c,w-1] ) / max(E_pdeaths[m][c,1:(w_start_resurgence[m] - 1)]) ;
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            ...
            log_r_pdeaths_counterfactual[m][c,:] = to_row_vector( normal_rng(xi_counterfactual[c][:,m], sigma_r_pdeaths[m]));
             E_pdeaths_counterfactual[m][c,1] = max(E_pdeaths[m][c,1:(w_start_resurgence[m] - 1)]) .* ( log_r_pdeaths_counterfactual[m][c,1] ) ;
            for(t in 2:T){
               E_pdeaths_counterfactual[m][c,t] = max(E_pdeaths[m][c,1:(w_start_resurgence[m] - 1)]) .* ( log_r_pdeaths_counterfactual[m][c,t] ) + E_pdeaths_counterfactual[m][c,t-1];
            }
