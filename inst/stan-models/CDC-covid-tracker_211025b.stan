functions {
  ...
  real sigmoid(real beta, real alpha, real t){
    return(1 / (1 + exp(-beta * (t - alpha))));
  }
  
  row_vector sigmoid_row_vector(real beta, real alpha, int M, row_vector t){
    return(rep_row_vector(1, M) ./ (1 + exp(-beta * (t - alpha))));
  }
...
data{
  ...
  int P;
  row_vector[P] prop_vac_sequence;
}
...
parameters {
  ...
  real vaccine_growth_intercept[C,C];
  real vaccine_mid_intercept[C,C];
  real vaccine_growth_slope[C,C];
  real vaccine_mid_slope[C,C];
...
transformed parameters {
  ...
  for(c in 1:C){
    intercept_resurgence[c] = rep_row_vector(intercept_resurgence0[c], M);
    slope_resurgence[c] = rep_row_vector(slope_resurgence0[c], M) ;

    for(c_prime in 1:C){
      intercept_resurgence[c] += sigmoid_row_vector(
                                  vaccine_growth_intercept[c,c_prime], vaccine_mid_intercept[c,c_prime], 
                                  M, prop_vac_start[c_prime]) .* rep_row_vector(vaccine_effect_intercept[c,c_prime], M);
                                  
      slope_resurgence[c] += sigmoid_row_vector(
                                  vaccine_growth_slope[c,c_prime], vaccine_mid_slope[c,c_prime], 
                                  M, prop_vac_start[c_prime]) .* rep_row_vector(vaccine_effect_slope[c,c_prime], M);
...
model {
  ...
  for(c in 1:C){
    ...
    vaccine_growth_intercept[c,:] ~ normal(10, 1);
    vaccine_mid_intercept[c,:] ~ normal(0.5, 0.5);
     
    vaccine_effect_slope[c,:] ~ normal(0,1);
    vaccine_growth_slope[c,:] ~ normal(10, 1);
    vaccine_mid_slope[c,:] ~ normal(0.5, 0.5);
...
generated quantities {
  ...
  row_vector[P] vaccine_effect_total_intercept[C,C];
  row_vector[P] vaccine_effect_total_slope[C,C];
  {
    int idx_log_lik = 0;

    for(c in 1:C){
        intercept_resurgence_counterfactual[c] = rep_row_vector(intercept_resurgence0[c], M) ;
        slope_resurgence_counterfactual[c] = rep_row_vector(slope_resurgence0[c], M) ;

        for(c_prime in 1:C){
            intercept_resurgence_counterfactual[c] += sigmoid_row_vector(
                                  vaccine_growth_intercept[c,c_prime], vaccine_mid_intercept[c,c_prime], 
                                  M, prop_vac_start[c_counterfactual]) .* rep_row_vector(vaccine_effect_intercept[c,c_prime], M);
                                  
            slope_resurgence_counterfactual[c] += sigmoid_row_vector(
                                  vaccine_growth_slope[c,c_prime], vaccine_mid_slope[c,c_prime], 
                                  M, prop_vac_start[c_counterfactual]) .* rep_row_vector(vaccine_effect_slope[c,c_prime], M);
                                  
            vaccine_effect_total_intercept[c, c_prime] = sigmoid_row_vector(
                                  vaccine_growth_intercept[c,c_prime], vaccine_mid_intercept[c,c_prime], 
                                  P, prop_vac_sequence) .* rep_row_vector(vaccine_effect_intercept[c,c_prime], P);
                                  
            vaccine_effect_total_slope[c, c_prime] = sigmoid_row_vector(
                                  vaccine_growth_slope[c,c_prime], vaccine_mid_slope[c,c_prime], 
                                  P, prop_vac_sequence) .* rep_row_vector(vaccine_effect_slope[c,c_prime], P);
        }
