...
transformed parameters {
  ...
  vector[A] mean_deaths_before_vaccination = rep_vector(0, A);
  ...
  for(a in 1:A){
    ...
    if( w_vaccination_start[a] == W)
      alpha[a,:] = ( phi[a,:] .* to_row_vector(lambda) )/ nu ;
      
    if( w_vaccination_start[a] < W){
      
      for(w in 1:(w_vaccination_start[a] - 1))
      {
        alpha[a,w] = phi[a,w] * lambda[w] / nu ;
    
        if(w >= w_vaccination_start[a] - 7){
          mean_deaths_before_vaccination[a] = mean_deaths_before_vaccination[a] + (phi[a,w] * lambda[w] / 7);
        }
      }
    
      for(w in w_vaccination_start[a]:W){
         alpha[a, w] = mean_deaths_before_vaccination[a] * susceptible[a,w] * phi[a,w] / nu ;
      }
    }
  }
