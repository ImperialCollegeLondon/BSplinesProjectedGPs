...
model {
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      log_r_pdeaths[m][c,1:T] ~ normal(log_xi[c][1:T,m] , sigma_r_pdeaths[m] );
    }
  }
...
generated quantities {
  ...
  for(m in 1:M){
    ...
    for(c in 1:C){
      ...
      log_r_pdeaths_predict[m][c,:] =  to_row_vector( normal_rng(log_xi[c][:,m], sigma_r_pdeaths[m]));
      r_pdeaths_predict[m][c,:] = exp(log_r_pdeaths_predict[m][c,:]);
  ...
  for(n in 1:N_COUNTERFACTUAL){
    ...
    for(m in 1:M){
      ...
      for(c in 1:C){
          ...
          log_r_pdeaths_counterfactual[m][c,:] = to_row_vector( normal_rng( log_xi_counterfactual[c][:,m], sigma_r_pdeaths[m]));
          r_pdeaths_counterfactual[m][c,:] = exp(log_r_pdeaths_counterfactual[m][c,:]);
