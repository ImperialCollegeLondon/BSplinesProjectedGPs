...
data{
  ...
  matrix[A, W] prop_vaccinated; // proportion of vaccinated
  int C; // number of vaccination coefficient
  int max_age_not_vaccinated; // the max age of non-vaccinated people 
  int<lower=1,upper=C> map_A_to_C[A-max_age_not_vaccinated];
  int I;
  int<lower=1,upper=I> I_age[C];
  int<lower=1,upper=I> index_prop[A,W];
...
parameters {
  ...
  row_vector[I_age[1]] gamma_re_1;
  row_vector[I_age[2]] gamma_re_2;
  real gamma0;
  real<lower=0> sd_gamma[I];
}
...
transformed parameters {
  ...
  matrix[C,I] gamma = gamma0 + append_row(append_col(gamma_re_1, rep_row_vector(0, I - I_age[1])), append_col(gamma_re_2, rep_row_vector(0, I - I_age[2]))); 
  matrix[A, W] susceptible = rep_matrix(rep_vector(0, A), W); 

  for(a in (max_age_not_vaccinated + 1):A){
    susceptible[a,:] = gamma[map_A_to_C[a - max_age_not_vaccinated], index_prop[a,:]] .*  prop_vaccinated[a,:];
...
model {
  ...
  gamma0 ~ normal(0,1);
  gamma_re_1 ~ normal(0,sd_gamma[1]);
  gamma_re_2 ~ normal(0,sd_gamma[2]);
