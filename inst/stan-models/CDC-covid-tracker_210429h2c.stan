functions {
  ...
  matrix gp(int N_rows, int N_columns, 
            real delta0,
            real alpha_gp_1, real alpha_gp_2, 
            matrix z1)
  {
    
    matrix[N_rows,N_columns] GP;
    
    matrix[N_rows, N_rows] K1 = alpha_gp_1^2 * diag_matrix(rep_vector(1.0, N_rows));
    matrix[N_rows, N_rows] L_K1;
    
    matrix[N_columns, N_columns] K2 = alpha_gp_2^2 *  diag_matrix(rep_vector(1.0, N_columns));
...
parameters {
  ...
  real<lower=0> alpha_gp_1;
  real<lower=0> alpha_gp_2;
  matrix[W,A] z1;
}
...
model {
  ...
  alpha_gp_1 ~ normal(0,1);
  alpha_gp_2 ~ normal(0,1);
