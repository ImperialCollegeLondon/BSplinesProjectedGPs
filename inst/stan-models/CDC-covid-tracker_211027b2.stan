...
data{
  ...
  row_vector[M] prop_vac_start_counterfactual[C];
}
...
generated quantities {
  ...
  matrix[C,T] perc_E_pdeaths_counterfactual[M]; 
  {
    ...
    for(c in 1:C){
        ...
        for(c_prime in 1:C){
            intercept_resurgence_counterfactual[c] += prop_vac_start_counterfactual[c] .* rep_row_vector(vaccine_effect_intercept[c,c_prime], M);
            slope_resurgence_counterfactual[c] += prop_vac_start_counterfactual[c] .* rep_row_vector(vaccine_effect_slope[c,c_prime], M);
        }
    ...
    for(m in 1:M){
        ...
        for(c in 1:C){
            ...
            E_pdeaths_counterfactual[m][c,:] = rep_row_vector(max(E_pdeaths[m][c,1:(w_start_resurgence[m] - 1)]), T) .* exp( log_r_pdeaths_counterfactual[m][c,:] );
