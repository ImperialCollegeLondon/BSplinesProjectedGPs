...
transformed parameters {
  ...
  matrix[A, W] susceptible = rep_matrix(pop, W) - 
    rep_matrix(append_row(rep_vector(0, max_age_not_vaccinated), gamma[map_A_to_C]), W) .* prop_vaccinated;

  for(w in 1:W)
  {
    
    phi[:,w] = softmax( log(susceptible[:,w]) + f[:,w] ); 
