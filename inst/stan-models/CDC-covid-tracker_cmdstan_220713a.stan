functions {
  ...
  real countries_log_dens(int[,] sum_count_censored, 
    ...
    for(c in 1:C){
      real lpmf2 = skew_normal_lpdf(log_r_pdeaths[c,1:T] | log_xi[c,1:T], sigma_r_pdeaths[m], skewness_r_pdeaths[m]);
      if(is_inf(lpmf2)){
        print("log_r_pdeaths[c,1:T]", log_r_pdeaths[c,1:T]);
        print("log_xi[c,1:T]", log_xi[c,1:T]);
        print("sigma_r_pdeaths[m]", sigma_r_pdeaths[m]);
        print("skewness_r_pdeaths[m]", skewness_r_pdeaths[m]);
        print("lpmf", skew_normal_lpdf(log_r_pdeaths[c,1:T] | log_xi[c,1:T], sigma_r_pdeaths[m], skewness_r_pdeaths[m]));
      }
      lpmf += skew_normal_lpdf(log_r_pdeaths[c,1:T] | log_xi[c,1:T], sigma_r_pdeaths[m], skewness_r_pdeaths[m]);
    }
  }
...
model {
  ...
  target += student_t_lpdf(skewness_r_pdeaths | 0.5, 0, 1);
