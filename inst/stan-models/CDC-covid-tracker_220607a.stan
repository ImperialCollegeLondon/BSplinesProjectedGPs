...
data{
  ...
  // time interval for ratio
  int period_idx_thresholds[4];
...
{   
    real delta0 = 1e-9;  
    int N_log_lik = 0;
    int length_first_period = period_idx_thresholds[2] - period_idx_thresholds[1] + 1;
    int length_second_period = period_idx_thresholds[4] - period_idx_thresholds[3] + 1;
...
transformed parameters {
  ...
  matrix[A, W] f[M];
  matrix[A,W] E_deaths[M];
  vector[A] deaths_first_period[M];
  vector[A] deaths_second_period[M];
  vector[A] deaths_ratio[M];
  ...
  for(m in 1:M){
    ...
    for(w in 1:W){
      ...
      E_deaths[m][:,w] = phi[m][:,w] * deaths_JHU[m,w];
