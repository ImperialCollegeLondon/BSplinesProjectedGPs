...
generated quantities{
  ...
  
  {
    ...
    for(c in 1:C){
      ...
      r_pdeaths_predict[c,:] = to_row_vector( lognormal_rng(log_xi[c,1:T], sigma_r_pdeaths[m]))  ;
  ...
  for(n in 1:N_COUNTERFACTUAL){
    ...
    log_xi_counterfactual = log(xi_counterfactual);
      ...
      for(c in 1:C){
          r_pdeaths_counterfactual[c,1:T] = to_row_vector( lognormal_rng( log_xi_counterfactual[c,1:T], sigma_r_pdeaths[m]) ) ;
